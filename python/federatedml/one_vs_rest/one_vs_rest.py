    ...
    def fit(self, data_instances=None, validate_data=None):
        ...
        for label_index, label in enumerate(self.classes):
            ...
            if self.has_label:
                ...
                if validate_data:
                    validate_mask_data = self._mask_data_label(validate_data, label=label)
                    validate_mask_data.schema['header'] = header
                else:
                    validate_mask_data = None
                LOGGER.info("finish mask label:{}".format(label))

                LOGGER.info("start classifier fit")
                classifier.fit_binary(data_instances_mask_label, validate_data=validate_mask_data)
            else:
                LOGGER.info("start classifier fit")
