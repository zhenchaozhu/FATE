...
from federatedml.param.boosting_tree_param import BoostingParam
...
class Boosting(ModelBase, ABC):
    def __init__(self):
        ...
        self.boosting_model_list = []  # list holds boosters
        ...
        self.cur_best_model = None  # cur best boosting model
    ...
    def convert_feature_to_bin(self, data_instance: DTable, handle_missing_value=False):
    ...
    def compute_loss(self, y_hat: DTable, y: DTable):
    ...
    def gen_feature_fid_mapping(schema):
        ...
        print("self.feature_name_fid_mapping is {}".format(feature_name_fid_mapping))
    ...
    def get_new_predict_score(self, y_hat: DTable, cur_sample_weights: DTable, dim=0):
    ...
    def prepare_data(self, data_inst: DTable):
    ...
    @abc.abstractmethod
    def check_label(self) -> typing.Tuple[typing.List[int], int, int]:
        """
        Returns: get classes indices, class number and booster dimension and class
        """
        raise NotImplementedError()

    def get_init_score(self, y: DTable, num_classes: int):
    ...
    def check_stop_condition(self, loss):
        ...
        if self.early_stopping > 0:
            if self.validation_strategy.is_best_performance_updated():
                self.cur_best_model = self.get_cur_model()
                LOGGER.debug('cur model saved')
            should_stop_a = self.validation_strategy.check_early_stopping(self.early_stopping)
    ...
    def score_to_predict_result(self, data_inst: DTable, y_hat: DTable):
