...
class HeteroDecisionTreeGuest(DecisionTree):
    ...
    def sync_encrypted_splitinfo_host(self, dep=-1, batch=-1, idx=-1):
        ...
        return encrypted_splitinfo_host if idx == -1 else [encrypted_splitinfo_host]
    ...
    def sync_tree(self):
        ...
        self.transfer_inst.tree.remote(self.tree_node,
                                       role=consts.HOST,
                                       idx=-1)
    ...
    def federated_find_split(self, dep=-1, batch=-1, idx=-1):
        ...
        encrypted_splitinfo_host = self.sync_encrypted_splitinfo_host(dep, batch, idx)

        for i in range(len(encrypted_splitinfo_host)):
            encrypted_splitinfo_host_table = session.parallelize(
                zip(self.cur_to_split_nodes, encrypted_splitinfo_host[i]), include_key=False,
                partition=self.data_bin._partitions)
