...
class FastFeatureHistogram(object):
    ...
    @staticmethod
    def table_format_transform(kv_iterator, bin_split_points, valid_features, use_missing):
        ...
        for nid, value in kv_iterator:
            ...
            for fid in range(len(valid_features)):
                ...
                if valid_features[fid]:
                    ...
                    histogram = [[0, 0, 0] for _ in range(feature_bin_num)]
                    for bid in range(len(bin_split_points[fid])):
                        ...
                        histogram[bid][0] = get_obj(grad)
                        histogram[bid][1] = get_obj(hess)
                        histogram[bid][2] = cnt
                    ...
                    if use_missing:
                        ...
                        histogram[-1][0] = get_obj(grad)
                        histogram[-1][1] = get_obj(hess)
                        histogram[-1][2] = cnt
