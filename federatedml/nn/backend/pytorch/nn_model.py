...
def build_optimzer(optim, model):
    if optim.optimizer == "Adam":
        return torch.optim.Adam(model.parameters(), optim.kwargs.get("learning_rate"))
    elif optim.optimizer == "SGD":
        return torch.optim.SGD(model.parameters(), optim.kwargs.get("learning_rate"))
    else:
        print("not support")


def restore_pytorch_nn_model(model_bytes):
    return PredictNN.restore_model(model_bytes)
